...
from models.configs import OptimizerType, LRSchedulerConfig, LRSchedulerType, resolve_optimizer, resolve_lr_scheduler, \
    resolve_metric
from models.elegansformer.elegansformer import ElegansFormerConfig, ElegansFormer, ElegansFormerOutput


class ElegansFormerVisionLossType(str, Enum):
    CROSS_ENTROPY = "cross_entropy"
    CUSTOM = "custom"


class ElegansFormerWrapper(lit.LightningModule):

    def __init__(self,
                 config: ElegansFormerConfig,
                 loss_type: ElegansFormerVisionLossType = ElegansFormerVisionLossType.CROSS_ENTROPY,
        ...
        self._loss_type: ElegansFormerVisionLossType = loss_type
        ...
        # Check for the number of labels from ElegansFormerConfig
        ...
        if loss_config is None:
            loss_config = {}
        if loss_type == ElegansFormerVisionLossType.CROSS_ENTROPY:
            if out_channels > 1:
                self._loss_fn = torch.nn.CrossEntropyLoss(**loss_config)
            else:
                self._loss_fn = torch.nn.BCEWithLogitsLoss(**loss_config)
        elif loss_type == ElegansFormerVisionLossType.CUSTOM:
        ...
        # Initialize ElegansFormer model from dictionary config
        self._elegansformer = ElegansFormer(config)
    ...
    @property
    def loss_type(self) -> ElegansFormerVisionLossType:
    ...
    @property
    def elegansformer(self) -> ElegansFormer:
        return self._elegansformer
    ...
    def configure_optimizers(self):
        ...
        # Get the parameters
        model_params = self.elegansformer.named_parameters()
    ...
    def _step(self, batch, batch_idx, step_type: str, *args, **kwargs) -> STEP_OUTPUT:
        ...
        # Call the model
        output: ElegansFormerOutput = self(
            x=inputs,
            attention_kwargs=attention_kwargs,
            *args,
            **kwargs
        ...
        if target.dtype == torch.long or self.loss_type == ElegansFormerVisionLossType.CROSS_ENTROPY:
