...
from models.configs import OptimizerType, LRSchedulerConfig, LRSchedulerType, resolve_optimizer, resolve_lr_scheduler, \
    resolve_metric
from models.elegans_vision.elegans_vision import VisionElegansConfig, VisionElegans, VisionElegansOutput
import pandas as pd
...
class ElegansVisionLossType(str, Enum):
    CROSS_ENTROPY = "cross_entropy"
    CUSTOM = "custom"


class VisionElegansWrapper(lit.LightningModule):

    def __init__(self,
                 config: VisionElegansConfig,
                 loss_type: ElegansVisionLossType = ElegansVisionLossType.CROSS_ENTROPY,
        ...
        self._loss_type: ElegansVisionLossType = loss_type
        ...
        self._elegansformer = VisionElegans(config)
    ...
    @property
    def loss_type(self) -> ElegansVisionLossType:
    ...
    @property
    def elegansformer(self) -> VisionElegans:
    ...
    def _step(self, batch, batch_idx, step_type: str, *args, **kwargs) -> STEP_OUTPUT:
        ...
        # Call the model
        output: VisionElegansOutput = self(
            x=inputs,
            attention_kwargs=attention_kwargs,
            *args,
            **kwargs
        ...
        if target.dtype == torch.long or self.loss_type == ElegansVisionLossType.CROSS_ENTROPY:
