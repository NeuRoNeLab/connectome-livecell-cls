...
from models.configs import OptimizerType, LRSchedulerConfig, LRSchedulerType, resolve_optimizer, resolve_lr_scheduler, \
    resolve_metric
from models.elegans_vision.elegans_convnext import ElegansConvNext, ElegansConvNextConfig, ElegansConvNextOutput


class ElegansConvNextLossType(str, Enum):
    CROSS_ENTROPY = "cross_entropy"
    CUSTOM = "custom"


class ElegansConvNextWrapper(lit.LightningModule):

    def __init__(self,
                 config: ElegansConvNextConfig,
                 loss_type: ElegansConvNextLossType = ElegansConvNextLossType.CROSS_ENTROPY,
        ...
        self._loss_type: ElegansConvNextLossType = loss_type
        ...
        # Check for the number of labels from CustomViTConfig
        ...
        # Initialize CustomViT model from dictionary config
        self._elegans_convnext = ElegansConvNext(config)
    ...
    @property
    def loss_type(self) -> ElegansConvNextLossType:
    ...
    @property
    def elegans_convnext(self) -> ElegansConvNext:
        return self._elegans_convnext
    ...
    def configure_optimizers(self):
        ...
        # Get the parameters
        model_params = self.elegans_convnext.named_parameters()
    ...
    def _step(self, batch, batch_idx, step_type: str, *args, **kwargs) -> STEP_OUTPUT:
        ...
        # Call the model
        output: ElegansConvNextOutput = self(
            x=inputs,
            attention_kwargs=attention_kwargs,
            *args,
            **kwargs
        ...
        if target.dtype == torch.long or self.loss_type == ElegansConvNextLossType.CROSS_ENTROPY:
