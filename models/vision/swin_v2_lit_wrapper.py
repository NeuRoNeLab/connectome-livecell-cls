...
from models.configs import OptimizerType, LRSchedulerConfig, LRSchedulerType, resolve_optimizer, resolve_lr_scheduler, \
    resolve_metric
from models.vision.swin_v2 import CustomSwinConfig, SwinTransformerV2, CustomSwinOutput


class CustomSwinLossType(str, Enum):
    CROSS_ENTROPY = "cross_entropy"
    CUSTOM = "custom"


class CustomSwinWrapper(lit.LightningModule):

    def __init__(self,
                 config: CustomSwinConfig,
                 loss_type: CustomSwinLossType = CustomSwinLossType.CROSS_ENTROPY,
        ...
        self._config: CustomSwinConfig = config
        self._loss_type: CustomSwinLossType = loss_type
        ...
        self._vit = SwinTransformerV2(config)

    @property
    def config(self) -> CustomSwinConfig:
        return self._config

    @property
    def loss_type(self) -> CustomSwinLossType:
    ...
    @property
    def vit(self) -> SwinTransformerV2:
        return self._vit
    ...
    def _step(self, batch, batch_idx, step_type: str, *args, **kwargs) -> STEP_OUTPUT:
        ...
        if target.dtype == torch.long or self.loss_type == CustomSwinLossType.CROSS_ENTROPY:
