...
from models.configs import OptimizerType, LRSchedulerConfig, LRSchedulerType, resolve_optimizer, resolve_lr_scheduler, \
    resolve_metric
from models.vision.mlpmixer import MLPMixer, MLPMixerConfig, MLPMixerOutput


class MLPMixerLossType(str, Enum):
    CROSS_ENTROPY = "cross_entropy"
    CUSTOM = "custom"


class MLPMixerWrapper(lit.LightningModule):

    def __init__(self,
                 config: MLPMixerConfig,
                 loss_type: MLPMixerLossType = MLPMixerLossType.CROSS_ENTROPY,
        ...
        self._config: MLPMixerConfig = config
        self._loss_type: MLPMixerLossType = loss_type
        ...
        self._mlpmixer = MLPMixer(config)
    ...
    @property
    def loss_type(self) -> MLPMixerLossType:
    ...
    @property
    def mlpmixer(self) -> MLPMixer:
        return self._mlpmixer
    ...
    def forward(self,
                x: torch.Tensor,
                *args, **kwargs) -> MLPMixer:
        return self.mlpmixer(
            x=x,
            *args,
            **kwargs
        )
    ...
    def configure_optimizers(self):
        ...
        # Get the parameters
        model_params = self.mlpmixer.named_parameters()
    ...
    def _step(self, batch, batch_idx, step_type: str, *args, **kwargs) -> STEP_OUTPUT:
        ...
        if target.dtype == torch.long or self.loss_type == MLPMixerLossType.CROSS_ENTROPY:
