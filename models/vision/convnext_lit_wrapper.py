...
from models.configs import OptimizerType, LRSchedulerConfig, LRSchedulerType, resolve_optimizer, resolve_lr_scheduler, \
    resolve_metric
from models.vision.convnext import CustomConvNextConfig, CustomConvNextOutput, ConvNeXt

class CustomConvNextLossType(str, Enum):
    CROSS_ENTROPY = "cross_entropy"
    CUSTOM = "custom"


class CustomConvNextWrapper(lit.LightningModule):

    def __init__(self,
                 config: CustomConvNextConfig,
                 loss_type: CustomConvNextLossType = CustomConvNextLossType.CROSS_ENTROPY,
        ...
        self._config: CustomConvNextConfig = config
        self._loss_type: CustomConvNextLossType = loss_type
        ...
        self._vit = ConvNeXt(config)

    @property
    def config(self) -> CustomConvNextConfig:
        return self._config

    @property
    def loss_type(self) -> CustomConvNextLossType:
    ...
    def forward(self,
                ...
                *args, **kwargs) -> CustomConvNextOutput:
        return self.vit(
            x=x,
    ...
    def configure_optimizers(self):
        ...
        # Get the parameters
        model_params = self.vit.named_parameters()
    ...
    def _step(self, batch, batch_idx, step_type: str, *args, **kwargs) -> STEP_OUTPUT:
        ...
        # Call the model
        output: CustomConvNextOutput = self(
            x=inputs,
            *args,
            **kwargs
        )
        ...
        if target.dtype == torch.long or self.loss_type == CustomConvNextLossType.CROSS_ENTROPY:
