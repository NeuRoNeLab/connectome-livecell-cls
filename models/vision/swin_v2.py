...
class CustomSwinOutput(NamedTuple):
    # Network output
    output: torch.Tensor
...
class CustomSwinConfig(SerializableConfig):
    ...
    def __init__(self,
        ...
        Args:
            ...
            use_as_encoder (bool): If True, the final classifier is not used. Default: False
        """
        super().__init__()
        ...
        # self.norm_layer: nn.Module = norm_layer
        self.ape: bool = ape
        self.patch_norm: bool = patch_norm
        self.use_checkpoint: bool = use_checkpoint
        self.pretrained_window_sizes: List[int] = pretrained_window_sizes
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.mlp = MLPSwin(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
...
class SwinTransformerV2(nn.Module):
    ...
    def __init__(self, config: CustomSwinConfig, **kwargs):
        super().__init__()

        self.config: CustomSwinConfig = config
        ...
        # stochastic depth depth decay rule
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(config.embed_dim * 2 ** i_layer),
                               ...
                               pretrained_window_size=config.pretrained_window_sizes[i_layer])
    ...
    def forward(self, x):
        x_flatten = self.forward_features(x)

        out = x_flatten
        if self.head is not None:
            out = self.head(x_flatten)

        swin_out = CustomSwinOutput(
            output=out,
            avg_embedding=x_flatten,
        )
        return swin_out
