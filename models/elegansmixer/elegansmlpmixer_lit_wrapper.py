...
from models.configs import OptimizerType, LRSchedulerConfig, LRSchedulerType, resolve_optimizer, resolve_lr_scheduler, \
    resolve_metric
from models.elegansmixer.elegansmlpmixer import ElegansMLPMixer, ElegansMLPMixerOutput, ElegansMLPMixerConfig


class ElegansMLPMixerLossType(str, Enum):
    CROSS_ENTROPY = "cross_entropy"
    CUSTOM = "custom"


class ElegansMLPMixerWrapper(lit.LightningModule):

    def __init__(self,
                 config: ElegansMLPMixerConfig,
                 loss_type: ElegansMLPMixerLossType = ElegansMLPMixerLossType.CROSS_ENTROPY,
        ...
        self._config: ElegansMLPMixerConfig = config
        self._loss_type: ElegansMLPMixerLossType = loss_type
        ...
        # Check for the number of labels from MLPMixerConfig
        ...
        # Initialize MLPMixer model from dictionary config
        self._elegansmlpmixer = ElegansMLPMixer(config)
    ...
    @property
    def loss_type(self) -> ElegansMLPMixerLossType:
    ...
    @property
    def elegansmlpmixer(self) -> ElegansMLPMixer:
        return self._elegansmlpmixer
    ...
    def forward(self,
                x: torch.Tensor,
                *args, **kwargs) -> ElegansMLPMixerOutput:
        return self.elegansmlpmixer(
            x=x,
            *args,
            **kwargs
        )
    ...
    def configure_optimizers(self):
        ...
        # Get the parameters
        model_params = self.elegansmlpmixer.named_parameters()
    ...
    def _step(self, batch, batch_idx, step_type: str, *args, **kwargs) -> STEP_OUTPUT:
        ...
        # Call the model
        output: ElegansMLPMixerOutput = self(
            x=inputs,
            *args,
            **kwargs
        )
        ...
        if target.dtype == torch.long or self.loss_type == ElegansMLPMixerLossType.CROSS_ENTROPY:
